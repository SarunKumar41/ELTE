...
del_firstlast x
| length x > 1 = init (drop 1 x)
= []

//Start = del_firstlast [1..10]
//Start = del_firstlast []
//Start = del_firstlast [1,3]
...
lastst :: [[Int]] -> [Int]
lastst [] = []
lastst [x:xs] = take 1 (reverse x) ++ lastst xs //[ hd (take 1 (reverse x)) : lastst xs]

//Start = lastst [[1,2,3],[5,6],[1],[7,8,9,10]]

//Start = hd (take 1 (reverse [1,2,3]))
